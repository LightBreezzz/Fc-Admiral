document.addEventListener('DOMContentLoaded', function () {
    ...
    // Бургер-меню для мобилок (выпадает из кнопки)
    ...
    if (mobileBurger && mobileMenu) {
        mobileBurger.addEventListener('click', (e) => {
            e.stopPropagation();
            mobileBurger.classList.toggle('mobile-burger--active');
            mobileMenu.classList.toggle('mobile-menu--open');
          });
    
        // Закрытие по клику вне меню
          document.addEventListener('click', (e) => {
            if (!mobileMenu.contains(e.target) && e.target !== mobileBurger && !mobileBurger.contains(e.target)) {
